...
admin-frontend_serverjs_setup_instancia() {
  print_banner
  printf "${WHITE} 💻 Criando server.js (Admin frontend na porta $portaadmin )...${GRAY_LIGHT}"
  printf "\n\n"
...
admin-frontend_node_dependencies_instancia() {
  ...
  sudo su - deploy <<EOF
  cd /home/deploy/$instancia/admin-frontend
  npm install
EOF
...
#   None
#######################################
admin-frontend_set_env_instancia() {
  print_banner
  printf "${WHITE} 💻 Configurando variáveis de ambiente (Admin frontend)...${GRAY_LIGHT}"
  printf "\n\n"
...
sudo su - deploy << EOF
  cat <<[-]EOF > /home/deploy/$instancia/admin-frontend/.env
URL_API=${backend_url}
URL_API2=${backend_url}
[-]EOF
EOF
...
server {
  server_name $admin_frontend_hostname;
    ...
    location / {
    proxy_pass http://127.0.0.1:$portaadmin;
    proxy_http_version 1.1;
    proxy_set_header Upgrade \$http_upgrade;
    proxy_set_header Connection 'upgrade';
    proxy_set_header Host \$host;
...
cat > /home/deploy/$instancia/admin-frontend/quasar.conf.js << 'END'
module.exports = function (ctx) {
  return {
     boot: [
      'vuelidate',
